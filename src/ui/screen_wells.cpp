...
#define WELLS_TOP_H   80
#define WELLS_CARD_H  ((SCREEN_H - WELLS_TOP_H - 16 * (NUM_WELLS + 1)) / NUM_WELLS)
...
lv_obj_t *screen_wells_create() {
	...
	lv_obj_set_style_text_font(lbl_global, &lv_font_montserrat_16, 0);
	lv_obj_set_style_text_color(lbl_global, COL_MUTED, 0);
	lv_obj_align(lbl_global, LV_ALIGN_BOTTOM_LEFT, 20, -10);

	lv_obj_t *bset = lv_btn_create(top);
	lv_obj_set_size(bset, 64, 48);
	lv_obj_align(bset, LV_ALIGN_RIGHT_MID, -84, 0);
	...
	lv_obj_set_style_text_font(lv_obj_get_child(bset, 0), &lv_font_montserrat_20, 0);

	lv_obj_t *bhelp = lv_btn_create(top);
	lv_obj_set_size(bhelp, 64, 48);
	lv_obj_align(bhelp, LV_ALIGN_RIGHT_MID, -16, 0);
	...
	lv_obj_set_style_text_font(lv_obj_get_child(bhelp, 0), &lv_font_montserrat_28, 0);
	...
	for (uint8_t i = 0; i < NUM_WELLS; i++) {
		...
		lv_obj_set_style_text_font(n, &lv_font_montserrat_40, 0);
		...
		lv_obj_align(s, LV_ALIGN_TOP_RIGHT, 0, 4);
		lbl_state[i] = s;
		...
		lv_obj_align(m, LV_ALIGN_LEFT_MID, 0, 10);
		lbl_metrics[i] = m;
