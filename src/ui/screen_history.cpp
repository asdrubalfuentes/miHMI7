...
lv_obj_t *screen_history_create() {
	...
	lv_obj_align(lbl_title, LV_ALIGN_TOP_LEFT, 20, 16);
	...
	lv_obj_align(lbl_sub, LV_ALIGN_TOP_LEFT, 20, 56);

	btn_toggle = lv_btn_create(scr);
	lv_obj_set_size(btn_toggle, 200, 56);
	lv_obj_align(btn_toggle, LV_ALIGN_TOP_RIGHT, -20, 46);
	...
	lv_obj_set_style_text_font(lv_obj_get_child(btn_toggle, 0), &lv_font_montserrat_20, 0);

	chart = lv_chart_create(scr);
	lv_obj_set_size(chart, SCREEN_W - 40, 260);
	lv_obj_align(chart, LV_ALIGN_TOP_MID, 0, 128);
	...
	lv_obj_set_style_text_font(mtitle, &lv_font_montserrat_20, 0);
	lv_obj_align(mtitle, LV_ALIGN_TOP_LEFT, 20, 404);

	lbl_month = lv_label_create(scr);
	lv_label_set_text(lbl_month, "-- m3");
	lv_obj_set_style_text_font(lbl_month, &lv_font_montserrat_40, 0);
	lv_obj_set_style_text_color(lbl_month, COL_TEAL, 0);
	lv_obj_align(lbl_month, LV_ALIGN_TOP_LEFT, 20, 428);

	lv_obj_t *back = lv_btn_create(scr);
	lv_obj_set_size(back, 200, 64);
	lv_obj_align(back, LV_ALIGN_BOTTOM_RIGHT, -20, -16);
